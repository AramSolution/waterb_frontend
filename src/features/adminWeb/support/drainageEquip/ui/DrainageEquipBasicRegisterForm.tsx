...
import { useDrainageEquipBasicRegister } from "../model/useDrainageEquipBasicRegister";
import { DrainageEquipDetailSection } from "./DrainageEquipDetailSection";
import "@/shared/styles/admin/register-form.css";

export interface DrainageEquipBasicRegisterFormProps {
  /** 상세·수정 URL의 `itemId` — 전달 시 GET detail로 폼·등록분 초기화(등록과 동일 편집 UI) */
  seedItemId?: string | null;
}

export const DrainageEquipBasicRegisterForm: React.FC<
  DrainageEquipBasicRegisterFormProps
> = ({ seedItemId = null }) => {
  ...
  if (seedItemId?.trim() && detailLoading) {
    return (
  ...
  return (
    <>
      ...
      >
        ...
        <DrainageEquipDetailSection
          key={
            seedItemId?.trim()
              ? `detail-${seedItemId.trim()}-${detailReloadKey}`
              : "register"
          }
          initialEntries={sewageInitialEntries}
          drainageEquipApi={drainageEquipApi}
