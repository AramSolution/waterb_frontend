...
import { type ScheduleWithApplyItem } from "@/widgets/userWeb/bizInfo2/scheduleWithApply";

function primaryTourLabel(item: ScheduleWithApplyItem, index: number): string {
  const country = String(item.item1 ?? "").trim();
  if (country) return country;
  const seq = Number(item.proSeq ?? 0);
  if (seq > 0) return `탐방 ${seq}`;
  return `탐방 ${index + 1}`;
}
...
function formatParticipationCount(item: ScheduleWithApplyItem): string {
  const preset = String(item.applyCntStr ?? "").trim();
  if (preset) return preset;
  const cap = String(item.recCnt ?? "").trim();
  if (!cap) return "";
  const applied =
    item.applyCnt != null && Number.isFinite(Number(item.applyCnt))
      ? Number(item.applyCnt)
      : 0;
  return `${applied}/${cap}`;
}
...
const EXPECTED_PRO_GB = "07";

/**
 * 글로벌 문화탐방(`proGb` 07) 상세. `bizInfo2` 껍데기 + `mainBg` 안 **탐방국가** 버튼·모달.
 */
const BizInfoGcSection: React.FC = () => {
  ...
  const [tourListLoading, setTourListLoading] = useState(false);
  const [tourListError, setTourListError] = useState<string | null>(null);
  ...
  const fetchDetail = useCallback(async (id: string) => {
    ...
    try {
      ...
      console.error("글로벌 문화탐방 상세 조회 실패:", e);
  ...
  const handleSubmitClick = () => {
    ...
    const targetUrl = `${BIZINPUT_GC_PATH}?${params.toString()}`;
    if (!isAuthenticated) {
  ...
  const openTourModal = useCallback(() => {
    ...
    if (!proId?.trim()) {
      setTourListError("사업 정보가 없습니다.");
      setScheduleList([]);
      return;
    }
    setTourListError(null);
    ...
    apiClient
      ...
      .catch(() => {
        setTourListError("탐방국가 목록을 불러오지 못했습니다.");
        setScheduleList([]);
      })
      .finally(() => setTourListLoading(false));
  }, [proId]);
  ...
  useEffect(() => {
    const handleEscape = (e: KeyboardEvent) => {
      if (e.key === "Escape" && tourModalOpen) setTourModalOpen(false);
    };
  ...
  return (
    <>
        ...
        <div className="rightContent">
          ...
          <div className="mainBg">
                ...
                <div className="detailBox">
                  ...
                  <div className="flex-sb">
                    <button
                      ...
                      id="openTourCountryModal"
                      aria-haspopup="dialog"
                      aria-controls="tourCountryModal"
            ...
            <div className="bizFile">
              <div className="title" id="fileDownloadTitleGc">
                첨부파일
              </div>
              <ul className="fileWrap" aria-labelledby="fileDownloadTitleGc">
      ...
      <div
        className={`modalOverlay ${tourModalOpen ? "active" : ""}`}
        id="tourCountryModalOverlay"
        aria-hidden={!tourModalOpen}
        onClick={(e) => e.target === e.currentTarget && closeTourModal()}
      >
        <div
          className="modalContent"
          role="dialog"
          aria-labelledby="tourCountryModalTitle"
          id="tourCountryModal"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="modalHeader">
            <div id="tourCountryModalTitle">탐방국가</div>
            <button
              type="button"
              className="closeBtn"
              aria-label="닫기"
          ...
          <div className="modalBody">
            {tourListLoading ? (
              <p className="loading">탐방국가 목록을 불러오는 중입니다.</p>
            ) : tourListError ? (
              <p className="error">{tourListError}</p>
            ) : scheduleList.length === 0 ? (
              <p className="empty">등록된 탐방국가 일정이 없습니다.</p>
            ) : (
                  ...
                  <li key={`${item.proSeq ?? i}-${i}`} className="bizItem">
                    <div className="topInfo">
                      <strong className="bizName">
                        {primaryTourLabel(item, i)}
                      </strong>
                    </div>
                    <dl className="infoList">
                      <dt>참여인원</dt>
                      <dd>{formatParticipationCount(item)}</dd>
                      <dt>지역</dt>
                      <dd>{cellText(item.item2)}</dd>
                      <dt>주제</dt>
                      <dd>{cellText(item.item3)}</dd>
                      <dt>일정</dt>
                      <dd>{cellText(item.item4)}</dd>
                    </dl>
                  </li>
                ))}
              </ul>
...
export default BizInfoGcSection;
