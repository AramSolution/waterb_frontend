...
/** 꿈틀꿈틀 우리아이 꿈탐험(06) 신청 페이지 */
const BIZINPUT_DM_PATH = "/userWeb/bizInputDm";
const BIZINFO_PR_PATH = "/userWeb/bizInfoPr";

/**
 * 꿈틀꿈틀 우리아이 꿈탐험(`proGb` 06) 상세 — `bizInfo2` 껍데기는 `BizInfoSection`과 동일.
 * 신청은 `bizInputDm`, `proGb !== 06`이면 `bizInfoPr`로 교체.
 */
const BizInfoDmSection: React.FC = () => {
  ...
  const fetchDetail = useCallback(async (id: string) => {
    ...
    try {
      ...
      console.error("꿈틀꿈틀 우리아이 꿈탐험 상세 조회 실패:", e);
  ...
  useEffect(() => {
    ...
    if (detail.proGb && detail.proGb !== "06") {
  ...
  const proGb = (detail?.proGb as string) ?? "06";
  ...
  const handleSubmitClick = () => {
    ...
    userWebSetTypeAndReqGbForBizNav(params, {
      ...
      fallbackType: searchParams.get("type"),
    ...
    const targetUrl = `${BIZINPUT_DM_PATH}${query}`;
    if (!isAuthenticated) {
  ...
  const wrongGbRedirect =
    detail != null &&
    !loading &&
    Boolean(detail.proGb && detail.proGb !== "06");
...
export default BizInfoDmSection;
