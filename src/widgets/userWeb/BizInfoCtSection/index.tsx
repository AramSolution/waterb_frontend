...
/** 공공형 진로진학 컨설팅(03) 신청 페이지 */
const BIZINPUT_CT_PATH = "/userWeb/bizInputCt";
const BIZINFO_PR_PATH = "/userWeb/bizInfoPr";

/**
 * 공공형 진로진학 컨설팅(`proGb` 03) 상세 — `bizInfo2` 껍데기는 `BizInfoSection`과 동일.
 * 신청은 `bizInputCt`, `proGb !== 03`이면 `bizInfoPr`로 교체.
 */
const BizInfoCtSection: React.FC = () => {
  ...
  const fetchDetail = useCallback(async (id: string) => {
    ...
    try {
      ...
      console.error("공공형 진로진학 컨설팅 상세 조회 실패:", e);
  ...
  useEffect(() => {
    ...
    if (detail.proGb && detail.proGb !== "03") {
  ...
  const proGb = (detail?.proGb as string) ?? "03";
  ...
  const handleSubmitClick = () => {
    ...
    const targetUrl = `${BIZINPUT_CT_PATH}${query}`;
    if (!isAuthenticated) {
  ...
  const wrongGbRedirect =
    detail != null &&
    !loading &&
    Boolean(detail.proGb && detail.proGb !== "03");
...
export default BizInfoCtSection;
