...
export interface BizInputDmSectionProps {
  proId?: string;
  fromMypage?: boolean;
...
/** Î∂ÄÎ™?Í¥ÄÍ≥?ÏΩîÎìú ???ºÎ≤® (F:Î∂Ä, M:Î™? E:Í∏∞Ì?) */
function formatGuardianRelation(relationCd?: string | null): string {
  const code = (relationCd ?? "").trim().toUpperCase();
  if (code === "F") return "Î∂Ä";
  if (code === "M") return "Î™?;
  if (code === "E") return "Í∏∞Ì?";
  return "";
}

/**
 * ÍøàÌ?ÍøàÌ? ?∞Î¶¨?ÑÏù¥ ÍøàÌÉê??06) ?ÑÏö© ?†Ï≤≠ ??
 * Î≥¥Ìò∏??Î≥∏Ïù∏?∏Ï¶ù(?∏Ï¶ù?òÍ∏∞) ??Ï≤?Üå???êÍ∏∞Í≥ÑÎ∞ú ?∞ÏàòÏßÄ??bizInput)Í≥??ôÏùº.
 * ?îÎ©¥ Íµ¨ÏÑ±: ?ôÎ?Î™®Ï†ïÎ≥?Í¥ÄÍ≥??¨Ìï®) ???ôÏÉù?ïÎ≥¥ ???ôÍµê?ïÎ≥¥ ??Ï≤®Î??åÏùº.
 * API: BY_STUDENT Î°úÎìú, POST /api/user/artappm/ (proGb=06).
 */
const BizInputDmSection: React.FC<BizInputDmSectionProps> = ({
  proId,
  fromMypage = false,
  initialReqEsntlId,
}) => {
  ...
  const [guardianRelationCd, setGuardianRelationCd] = useState<string>("");

  const [guardianName, setGuardianName] = useState("");
  const [guardianContact, setGuardianContact] = useState("");
  const [guardianBirth, setGuardianBirth] = useState("");
  /** Í¥ÄÍ≥??úÏãúÎ™?(API RELATION_GB_NM ?∞ÏÑ†, ?ÜÏúºÎ©?relationCdÎ°??¨Îß∑) */
      ...
      apiClient
        ...
        .then((res) => {
          ...
          UserArmuserService.getDetail(first)
            .then((r) => {
              const d = r.detail as
                | (typeof r.detail & { ihidnum?: string; relationCd?: string })
                | null;
              if (!d) return;
              ...
              setGuardianRelationCd((d as { relationCd?: string }).relationCd ?? "");
              setGuardianRelationNm(
                String(
          ...
          <div className="registrationContainer bizInput">
            <form className="mainForm" onSubmit={handleSubmit}>
              <section
                ...
                aria-labelledby="bizDmGuardianSectionTitle"
              >
                <div className="sectionHeader">
                  <div className="titleWrapper">
                    <div
                      className="sectionTitle"
                      id="bizDmGuardianSectionTitle"
                    >
                      ?ôÎ?Î™®Ï†ïÎ≥?
                    </div>
                    <span
                ...
                <div className="formGrid bizInput">
                  ...
                  <div className="formRow split">
                    ...
                    <div className="fieldUnit">
                      <label htmlFor="guardianRelation" className="formLabel">
                        Í¥ÄÍ≥?
                      </label>
                      <div className="formControl">
                        <input
                          ...
                          value={
                            guardianRelationNm ||
                            formatGuardianRelation(guardianRelationCd)
                          }
                          readOnly
                          aria-readonly="true"
                          aria-label="Î∂ÄÎ™?Í¥ÄÍ≥?
                        />
                      </div>
                    </div>
                  </div>
              ...
              <section className="formSection">
                ...
                <div className="formGrid bizInput">
                  <div className="formRow">
                    <span className="formLabel" id="lblSchoolNameDm">
                      ?ôÍµêÎ™?
                    </span>
                  ...
                  <div className="formRow">
                    ...
                    <div className="formControl fileListContainer">
                                ...
                                {canSaveOrApply && (
                                  <button
                                    ...
                                    onClick={(ev) => {
                                      ev.preventDefault();
                                      ev.stopPropagation();
                                      removeExistingFile(f.fileId, f.seq);
                                    }}
                                    aria-label={`${f.orgfNm ?? "?åÏùº"} ??†ú`}
                                  >
                                    <img
                      ...
                      {pendingFiles.length > 0 && (
                        <ul
                          className="fileList"
                          aria-label="Ï∂îÍ???Ï≤®Î??åÏùº Î™©Î°ù"
                        >
                          {pendingFiles.map((p) => (
                            <li key={p.id} className="fileItem">
                              <span className="fileNm">{p.file.name}</span>
                              {canSaveOrApply && (
                                <button
                                  type="button"
                                  className="btnFileDel"
                                  onClick={() => removePendingFile(p.id)}
                                  aria-label={`${p.file.name} ?úÍ±∞`}
                                >
                                  <img
                                    src={`${ICON}/ico_file_del.png`}
                                    alt=""
...
export default BizInputDmSection;
