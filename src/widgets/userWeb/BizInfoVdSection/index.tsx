...
/** 1:1 원어민 화상영어(04) 신청 페이지 */
const BIZINPUT_VD_PATH = "/userWeb/bizInputVd";
const BIZINFO_PR_PATH = "/userWeb/bizInfoPr";

/**
 * 1:1 원어민 화상영어(`proGb` 04) 상세 — `bizInfo2` 껍데기는 `BizInfoSection`과 동일.
 * 신청은 `bizInputVd`, `proGb !== 04`이면 `bizInfoPr`로 교체.
 */
const BizInfoVdSection: React.FC = () => {
  ...
  const fetchDetail = useCallback(async (id: string) => {
    ...
    try {
      ...
      console.error("1:1 원어민 화상영어 상세 조회 실패:", e);
  ...
  useEffect(() => {
    ...
    if (detail.proGb && detail.proGb !== "04") {
  ...
  const proGb = (detail?.proGb as string) ?? "04";
  ...
  const handleSubmitClick = () => {
    ...
    const targetUrl = `${BIZINPUT_VD_PATH}${query}`;
    if (!isAuthenticated) {
  ...
  const wrongGbRedirect =
    detail != null &&
    !loading &&
    Boolean(detail.proGb && detail.proGb !== "04");
...
export default BizInfoVdSection;
