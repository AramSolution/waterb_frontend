...
function extractYnFromResultData(
...
/** 관리자 회원 폼 등: GPKI·감면 조회 API (기존 사용자웹 엔드포인트와 동일) */
export class AdminGpkiCertService {
  ...
  static async checkCitizenByResideCode(
    ...
    try {
      ...
      return {
        ...
        message: isCitizen ? "군산 시민으로 확인되었습니다." : "해당안함",
        raw: response,
      };
    } catch (error) {
      if (error instanceof ApiError) {
        return {
          ...
          reason: "service_unavailable",
          message:
            "시민인증 서비스 오류가 발생했습니다. 잠시 후 다시 시도해주세요.",
          raw: {
            resultCode: String(error.status || "ERROR"),
            resultData: error.data,
          },
